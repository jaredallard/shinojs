...
class Classifier extends Natural.LogisticRegressionClassifier {
  ...
  /**
   * Add sample text to the classifier module.
   *
   * @param {String} [sample=''] Sample text
   * @param {String} label       Label of text
   * @returns {Natural.Classifier} addDocument response
   */
  add(sample = '', label) {
  ...
  /**
   * Run text against the classifier.
   *
   * @param  {String} text Text to test the classifier on.
   * @return {Natural.classifierResult}      Object of classifiers with a numeric 'score'
   */
  run(text) {
