...
const getContext = content => {
  return _.get(users, `${content.sender.screen_name}.context`);
}
 ...
 * @return {String}         Method Name
 */
const getLastContext = content => {
  return _.get(users, `${content.sender.screen_name}.last`);
}
...
const addValidator = (validator, root) => {
  ...
  // default action = address
  if(!validator.action) innerValidator.action = address
...
const runMiddleware = (T, content) => {
  ...
  } else { // no context, just filter out non-root methods
