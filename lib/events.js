 ...
 * @type {natural}
 */
events.classifier = new natural.LogisticRegressionClassifier();
...
const setContext = (content, lastMethod) => {
  const username = content.sender.screen_name
  users[username] = lastMethod
}
 ...
 *
 * @param  {Object} content Twitter Object
 * @return {Sring}          Method name
 */
const getContext = content => {
  return _.get(users, content.sender.screen_name);
}

/**
 * Get a list of available methods from context
 * and it's defaults.
 *
 * @param  {String} method   Method
 * @return {Array}           List of methods available.
 */
const getAvailableMethods = method => {
...
const addValidator = (validator, root) => {
  if(root) root += '.' // append the dot notation
  if(!root) root = ''

  // Generate a base address if not supplied
  let address = validator.address
  if(!address) address = randStr({ length: 10 });
  ...
  // train classifier module
  if(validator.classifiers) {
    validator.classifiers.forEach(sample => {
      events.classifier.addDocument(sample, finalAddress)
    })
  }
...
const runMiddleware = (T, content) => {
  const classifier = events.classifier
  ...
  debug('nlp:sentiment', sentimentResult)
  content.analysis.classifier = {
    classifications: classifierResult,
    classified: classified
  }
  content.analysis.sentimentRating = sentimentResult

  if(context) {
    debug('nlp:hasContext', context)

    // filter out methods that don't contain our method string
    availableClassifiers   = classifierResult.filter(c => {
      return c.label.indexOf(`${context}.`) !== -1 ? true : false // translate to true/false
    })
  } else { // no context, just filter out root method
    availableClassifiers = classifierResult.filter(c => { return c.label.indexOf('.') !== -1 ? false : true})
  }

  debug('nlp:availableClassifiers', availableClassifiers)

  const highestClassifier = availableClassifiers[0]

  const address           = highestClassifier.label
  if(highestClassifier.value < 0.7) {
    // invalidate the context.
    setContext(content, null)

    // TODO: Handle previous context's default "wishes"

    return debug('nlp:score', `picked: ${address} to low`);
  }
