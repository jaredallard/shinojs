...
const runMiddleware = (T, array, content) => {
  async.eachSeries(array, async (listener, done) => {
    ...
    // VERBOSE: debug('listener:process', listener)
    ...
    if(classifier) {
      ...
      if(classified !== label) {
        debug('nlp:classified', 'not matching', label)
        return done()
      }

      const classificationObject = classifierResult.filter(c => { return c.label == label});
      const classificationScore = classificationObject[0].value

      debug('nlp:label', 'score is', classificationScore)
      if(classificationScore < 0.7) {
        debug('nlp:error', 'score is below threshold')
        return done()
      }
    }
    ...
    if(validatorType === 'function') {
      ...
      // test the validator
      if(!validator.test(content.text)) {
        debug('validator', 'no match')
        return done()
      }
    }
 ...
 */
events.addListener = function(type, validator, functions) {
  ...
  if(isNLP) {
    ...
    validator.classifiers.forEach(sample => {
      this.classifier.addDocument(sample, label)
    })
